...
export default function Body() {
    ...
    return(
       ...
       <nav className="bg-neutral-800/70 px-[2vw]  w-full md:w-auto fixed bottom-0 left-0 md:left-auto md:bottom-auto md:top-[1vh] md:right-[6vw] z-50 
  ...
  <div className="container mx-auto flex justify-between md:justify-center items-center py-1">
    <ul className="flex justify-between space-x-2 w-full md:w-auto md:space-x-10 font-[600] text-xs sm:text-sm md:text-base cursor-pointer transition-colors duration-200 ease-in-out">
      <li><p onClick={() => setPage('about')} className={page === 'about' ? 'text-yellow-400' : 'hover:text-yellow-500'}>About</p></li>
      <li>
  <p
    onClick={() => setPage('resume')}
    className={page === 'resume' ? 'text-yellow-400' : 'hover:text-yellow-500'}
  >
    ...
    <span className="hidden md:inline">Experience</span>
      ...
      <li><p onClick={() => setPage('blog')} className={page === 'blog' ? 'text-yellow-400' : 'hover:text-yellow-500'}>Blogs</p></li>
      <li><p onClick={() => setPage('contact')} className={page === 'contact' ? 'text-yellow-400' : 'hover:text-yellow-500'}>Contact</p></li>
      <li><a href="https://drive.google.com/file/d/1HoEsCKMQsOjHcNXo4QqfjdkfxZ_43gg7/view?usp=sharing" target="_blank" rel="noopener noreferrer" className="text-white hover:text-yellow-500">
        <p>Resume</p>
      </a></li>
    </ul>
  </div>
